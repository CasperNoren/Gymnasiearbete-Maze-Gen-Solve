...
function Cell(i, j){
  ...
  this.checkNeighbors = function(){
    var neighbors = [];
    if(j > 0){var top = grid[this.i][this.j - 1];}else{var top = grid[-1]}
    if(i < cols - 1){var right = grid[this.i + 1][this.j];}else{var right = grid[-1]}
    if(j < rows - 1){var bottom = grid[this.i][this.j + 1];}else{var bottom = grid[-1]}
    if(i > 0){var left = grid[this.i - 1][this.j];}else{var left = grid[-1]}
...
function removeExtraWalls(){
  for(var i = 0; i < (cols * rows) * (percentOfWallsToRemove * 0.01); i++){
    ...
    if(b){ //Fail safe incase the function returns undefined
      removeWalls(a, b)
    } else {
      i--;
    }
...
function startProgram() {
  ...
  pBtn.id = 'pauseBtn';
  pBtn.addEventListener('click', function(){ doPause() }, false);
